...
public abstract class Scrolling
{
    ...
    public static void scroll(JComponent c, Rectangle r, int horizontalBias,
            int verticalBias) {
        Rectangle visible = c.getVisibleRect(), dest = new Rectangle(r);

        if (dest.width > visible.width) {
            if (horizontalBias == UNCHANGED) {
                if (dest.x <= visible.x
        ...
        if (dest.height > visible.height) {
           if (verticalBias == UNCHANGED) {
                if (dest.y <= visible.y
                        && dest.y + dest.height >= visible.y + visible.height) {
                    dest.y = visible.y;
